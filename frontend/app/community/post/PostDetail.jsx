...
export default function PostDetail() {
  ...
  // 1. 게시글 및 댓글 상세 조회 (팀이 정한 주소 사용)
  useEffect(() => {
    const fetchPostDetail = async () => {
      try {
        setIsLoading(true);
        const response = await client.get(`/api/community/post/${id}`);
  ...
  // 2. 댓글 등록 함수 (댓글 좋아요 기능 없음)
  const handleCommentSubmit = async () => {
    if (!commentText.trim()) return;

    try {
      const response = await client.post('/api/community/comment', {
        postId: id,
        content: commentText,
        author: 'TestUser' 
      });
