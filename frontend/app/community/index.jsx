...
import { useCallback, useMemo, useState } from 'react'; // useEffect 미사용시 제거
import {
  FlatList,
  Platform,
  StatusBar,
...
// ✅ 1. client 대신 우리가 만든 communityApi에서 함수를 가져옵니다.
...
export default function CommunityScreen() {
  ...
  // ✅ 2. 서버에서 데이터 가져오는 함수를 API 함수 사용으로 수정
  const fetchPosts = useCallback(async () => {
    setIsLoading(true);
    try {
      // client.get 대신 미리 정의한 getPostList() 호출
      const result = await getPostList();
      
      // 팀장님 규칙: result.code === "SUCCESS" 확인 및 데이터 할당
  ...
  const renderItem = ({ item }) => (
    <TouchableOpacity
      ...
      // 명세서상 ID가 postId이므로 이를 사용 (기존과 동일)
    ...
    <SafeAreaView style={styles.container}>
      ...
      {isLoading ? (
        ...
        <FlatList
          ...
          // ✅ 3. keyExtractor에서 item.postId 사용 (item.id 아님)
