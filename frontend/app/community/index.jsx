...
import {
  globalBookmarkState,
  globalCommentCountState,
  globalCommentStore,
  globalLikeCountState,
  globalViewCountState,
} from './communityState';
...
export default function CommunityScreen() {
  ...
  const [posts, setPosts] = useState([
    ...Array.from({ length: 25 }, (_, i) => ({
      id: `${i + 1}`,
      title: i === 0 ? '리액트 네이티브 너무 재밌네요!' : `게시글 ${i + 1}번 제목입니다.`,
    ...
    useCallback(() => {
      setPosts((currentPosts) =>
        currentPosts.map((post) => {
          const realLikes = globalLikeCountState?.[post.id] ?? post.likes;
          const realViews = globalViewCountState?.[post.id] ?? post.views;
          const realComments =
            globalCommentCountState?.[post.id] ??
            (globalCommentStore?.[post.id] ? globalCommentStore[post.id].length : post.commentCount);
          ...
          return {
            ...
            displayBookmarks: globalBookmarkState[post.id] ? post.bookmarks + 1 : post.bookmarks,
          };
        }),
      );
    }, []),
  ...
  const filteredPosts = useMemo(() => {
    ...
    if (activeTab === '북마크') return posts.filter((p) => globalBookmarkState[p.id]);
    return posts;
  }, [activeTab, posts]);
      ...
      <View style={styles.postFooterRow}>
        <View style={styles.postStatsGroup}>
          <View style={styles.statItem}>
            ...
            <Text style={[styles.statText, { color: '#5AA9E6' }]}>{item.commentCount}</Text>
          </View>

          <View style={styles.statItem}>
            <Heart size={14} color="#FF5A5F" />
            <Text style={[styles.statText, { color: '#FF5A5F' }]}>{item.likes}</Text>
          </View>
          <View style={styles.statItem}>
            <Bookmark size={14} color="#FFD700" />
            <Text style={[styles.statText, { color: '#FFD700' }]}>{item.displayBookmarks ?? item.bookmarks}</Text>
          </View>
        </View>
      </View>
    </TouchableOpacity>
    ...
    return (
      <View style={styles.paginationContainer}>
        ...
        <TouchableOpacity
          ...
          style={styles.arrowBtn}
        >
          <ChevronRight size={20} color={currentPage === totalPages ? '#ccc' : '#333'} />
        </TouchableOpacity>
      </View>
    );
  };
    ...
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.topTabContainer}>
        {tabs.map((tab) => (
          ...
          >
            <Text style={[styles.topTabItem, activeTab === tab && styles.activeTabText]}>{tab}</Text>
          </TouchableOpacity>
        ))}
      </View>
        ...
        ListEmptyComponent={
          <View style={styles.emptyContainer}>
            <Text style={styles.emptyText}>
              {activeTab === '북마크' ? '북마크한 게시글이 없습니다.' : '게시물이 없습니다.'}
            </Text>
          </View>
        }
      />
...
const styles = StyleSheet.create({
  ...
  topTabContainer: { flexDirection: 'row', borderBottomWidth: 1, borderBottomColor: '#eeeeee' },
  tabButton: {
  ...
  postTitle: { fontSize: 16, fontWeight: '800', color: '#333333', marginBottom: 16, lineHeight: 22 },
  postFooterRow: {
    flexDirection: 'row',
    justifyContent: 'space-between',
    alignItems: 'center',
  ...
  commentCountText: { fontSize: 13, color: '#5AA9E6', marginLeft: 4, fontWeight: '700' },
  paginationContainer: {
  ...
  pageBtn: {
    ...
    marginHorizontal: 4, borderRadius: 16,
  },
  ...
  fab: {
    ...
    borderRadius: 28, backgroundColor: '#5AA9E6', alignItems: 'center', justifyContent: 'center',
    ...Platform.select({
