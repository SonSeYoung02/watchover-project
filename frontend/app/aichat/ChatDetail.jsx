...
const ChatDetail = () => {
  ...
  const { id } = route.params || {}; // History에서 넘겨받은 chatingRoomId
  ...
  useEffect(() => {
    const fetchDetail = async () => {
      try {
        setIsLoading(true);
        // 명세서 기반: GET /api/chatBot/{chatingRoomId}
        const response = await client.get(`/api/chatBot/${id}`);
        if (response.data && response.data.data) {
          // 서버 데이터 구조에 따라 chatLogs 또는 messages 추출
          setChatLogs(response.data.data.chatLogs || []);
        }
      } catch (error) {
