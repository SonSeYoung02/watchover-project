...
const AiChat = () => {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      if (!roomId) {
        const roomRes = await client.post('/api/chatBot/0');
        roomId = roomRes.data.data.chatingRoomId;
        setCurrentRoomId(roomId);
      }

      const response = await client.post(`/api/chatBot/${roomId}`, {
      ...
      setMessages((prev) => [
        ...prev,
        { id: Date.now() + 1, type: 'ai', text: '연결 상태가 불안정합니다. 잠시 후 다시 시도해주세요.' },
      ]);
    } finally {
      setIsWaiting(false);
    }
  ...
  // 2. 상담 종료 및 감정 요약 함수 (handleSend 밖으로 꺼냄)
      ...
      [
        ...
        { 
          ...
          onPress: async () => {
            try {
              setIsWaiting(true);
              const response = await client.post(`/api/calendar/emotion/${currentRoomId}`);
              
              if (response.data && response.data.code === "SUCCESS") {
                const todayEmotion = response.data.data.emotion || "평온";
                Alert.alert(
                  "오늘의 마음 기록",
                  `오늘 당신의 마음은 '${todayEmotion}'이었군요. 기록이 달력에 저장되었습니다.`,
                  [{ text: "확인", onPress: () => navigation.navigate('Home') }]
                );
              }
            } catch (error) {
              console.error("감정 요약 실패:", error);
              Alert.alert("알림", "감정 기록 중 오류가 발생했지만 상담은 종료되었습니다.");
