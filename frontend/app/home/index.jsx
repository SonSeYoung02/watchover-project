...
import client from '../../api/client'; // 통신 도구 임포트
...
const MainHome = () => {
  ...
  useEffect(() => {
    const fetchQuotes = async () => {
      try {
        const response = await client.get('/api/banners/1'); // 첫 번째 명언 호출
        if (response.data && response.data.data) {
          // 명세서 변수명이 message라면 mapping
          const serverQuote = {
            id: 1,
            text: response.data.data.message || '데이터가 없습니다.',
            author: 'Care AI',
            accent: '#5AA9E6',
          };
          setQuotes([serverQuote]); // Swiper에 넣기
        }
      } catch (error) {
